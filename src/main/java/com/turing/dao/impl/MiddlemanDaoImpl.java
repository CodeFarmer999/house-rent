 ...
 * @date 2022/5/11 9:09
 **/
public class MiddlemanDaoImpl extends BaseDaoImpl<User> implements MiddlemanDao {
    ...
    public boolean addMiddleman(User user) {
        ...
        try {
            ...
            if (user1 == null) {
                ...
                UserRole userRoleInsert = C3P0Utils.getQueryRunner().insert(sql2, new BeanHandler<>(UserRole.class), userProxy.getId(), 5);
    ...
    public boolean deleteMiddleman(int id) {
        ...
        for (UserRole userRole : query) {
            if (userRole != null && userRole.getRoleId() == 5 && query.size() > 1) {
                String sql2 = "delete from user_role where role_id = ? and user_id = ?";
                try {
                    updateCount = C3P0Utils.getQueryRunner().update(sql2, 5, id);
                    return updateCount > 0;
                } catch (SQLException e) {
                    e.printStackTrace();
                }
            } else if (userRole != null && userRole.getRoleId() == 5 && query.size() == 1) {
                try {
    ...
    public boolean updateMiddleman(User user) {
        String sql = "update user set username = ?, password = ?, phone = ?, name = ? where id = ? and id in (select user_id from user_role where role_id = 5)";
        try {
    ...
    public User getMiddleman(int id) {
        String sql = "select * from user where id = ? and id in (select user_id from user_role where role_id = 5)";
        try {
    ...
    public List<User> getMiddlemans() {
        String sql = "select * from user where id in (select user_id from user_role where role_id = 5)";
        try {
