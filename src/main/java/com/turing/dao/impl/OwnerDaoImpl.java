 ...
 * @date 2022/5/11 9:09
 **/
public class OwnerDaoImpl extends BaseDaoImpl<User> implements OwnerDao {
    ...
    public boolean addOwner(User user) {
        ...
        try {
            ...
            if (user1 == null) {
                ...
                UserRole userRoleInsert = C3P0Utils.getQueryRunner().insert(sql2, new BeanHandler<>(UserRole.class), userProxy.getId(), 4);
    ...
    public boolean deleteOwner(int id) {
        ...
        for (UserRole userRole : query) {
            if (userRole != null && userRole.getRoleId() == 4 && query.size() > 1) {
                String sql2 = "delete from user_role where role_id = ? and user_id = ?";
                try {
                    updateCount = C3P0Utils.getQueryRunner().update(sql2, 4, id);
                    return updateCount > 0;
                } catch (SQLException e) {
                    e.printStackTrace();
                }
            } else if (userRole != null && userRole.getRoleId() == 4 && query.size() == 1) {
                try {
    ...
    public boolean updateOwner(User user) {
        String sql = "update user set username = ?, password = ?, phone = ?, name = ? where id = ? and id in (select user_id from user_role where role_id = 4)";
        try {
    ...
    public User getOwner(int id) {
        String sql = "select * from user where id = ? and id in (select user_id from user_role where role_id = 4)";
        try {
    ...
    public List<User> getOwners() {
        String sql = "select * from user where id in (select user_id from user_role where role_id = 4)";
        try {
